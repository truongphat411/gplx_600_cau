// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'question.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

_$QuestionImpl _$$QuestionImplFromJson(Map<String, dynamic> json) =>
    _$QuestionImpl(
      Z_PK: (json['Z_PK'] as num?)?.toInt(),
      ZQUESTIONCONTENT: json['ZQUESTIONCONTENT'] as String?,
      ZIMAGE: json['ZIMAGE'] as String?,
      ZOPTION1: json['ZOPTION1'] as String?,
      ZOPTION2: json['ZOPTION2'] as String?,
      ZOPTION3: json['ZOPTION3'] as String?,
      ZOPTION4: json['ZOPTION4'] as String?,
      ZANSWERIMG: json['ZANSWERIMG'] as String?,
      ZANSWERDESC: json['ZANSWERDESC'] as String?,
      ZCORRECT: (json['ZCORRECT'] as num?)?.toInt(),
      ZQUESTIONTYPE: (json['ZQUESTIONTYPE'] as num?)?.toInt(),
      ZLEARNED: (json['ZLEARNED'] as num?)?.toInt(),
      ZMARKED: (json['ZMARKED'] as num?)?.toInt(),
      ZWRONG: (json['ZWRONG'] as num?)?.toInt(),
      INGROUP_A1: (json['INGROUP_A1'] as num?)?.toInt(),
      INGROUP_A: (json['INGROUP_A'] as num?)?.toInt(),
      INGROUP_B1: (json['INGROUP_B1'] as num?)?.toInt(),
      INGROUP_B: (json['INGROUP_B'] as num?)?.toInt(),
      DIGROUPA1: json['DIGROUPA1'] as String?,
      DIGROUPB1: json['DIGROUPB1'] as String?,
      INGROUP_DTOE: (json['INGROUP_DTOE'] as num?)?.toInt(),
      ZQUESTIONDIE: json['ZQUESTIONDIE'] as String?,
      Z_ENT: (json['Z_ENT'] as num?)?.toInt(),
      ZLATEST: (json['ZLATEST'] as num?)?.toInt(),
      REZ1: (json['REZ1'] as num?)?.toInt(),
    );

Map<String, dynamic> _$$QuestionImplToJson(_$QuestionImpl instance) =>
    <String, dynamic>{
      'Z_PK': instance.Z_PK,
      'ZQUESTIONCONTENT': instance.ZQUESTIONCONTENT,
      'ZIMAGE': instance.ZIMAGE,
      'ZOPTION1': instance.ZOPTION1,
      'ZOPTION2': instance.ZOPTION2,
      'ZOPTION3': instance.ZOPTION3,
      'ZOPTION4': instance.ZOPTION4,
      'ZANSWERIMG': instance.ZANSWERIMG,
      'ZANSWERDESC': instance.ZANSWERDESC,
      'ZCORRECT': instance.ZCORRECT,
      'ZQUESTIONTYPE': instance.ZQUESTIONTYPE,
      'ZLEARNED': instance.ZLEARNED,
      'ZMARKED': instance.ZMARKED,
      'ZWRONG': instance.ZWRONG,
      'INGROUP_A1': instance.INGROUP_A1,
      'INGROUP_A': instance.INGROUP_A,
      'INGROUP_B1': instance.INGROUP_B1,
      'INGROUP_B': instance.INGROUP_B,
      'DIGROUPA1': instance.DIGROUPA1,
      'DIGROUPB1': instance.DIGROUPB1,
      'INGROUP_DTOE': instance.INGROUP_DTOE,
      'ZQUESTIONDIE': instance.ZQUESTIONDIE,
      'Z_ENT': instance.Z_ENT,
      'ZLATEST': instance.ZLATEST,
      'REZ1': instance.REZ1,
    };
